...
export function DepositHistory({ walletAddress, limit = 10 }: DepositHistoryProps) {
  ...
  useEffect(() => {
    if (!walletAddress || !isConnected || !isEnabled) return;

    const unsubscribe = indexerService.subscription.subscribeToDeposits(walletAddress, {
      onInsert: (deposit) => {
        // Update cache
        queryClient.setQueryData<{ data: Deposit[]; total: number; hasMore: boolean }>(
          ['deposits', walletAddress, limit],
          (old) => {
            if (!old) return { data: [deposit], total: 1, hasMore: false };
            return {
              ...old,
              data: [deposit, ...old.data].slice(0, limit),
              total: old.total + 1,
            };
          }
        );
    ...
    return unsubscribe;
  }, [walletAddress, isConnected, isEnabled, queryClient, limit]);
